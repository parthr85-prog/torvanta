...
export default function ContractorProfile() {
  ...
  const confirmOtp = async () => {
    try {
      ...
      setIsVerified(true);
      Alert.alert("Verified. You can now edit.");
    } catch {
      Alert.alert("Invalid OTP");
    }
  };

  /* ---------------- GST VERIFY OR RE-VERIFY ---------------- */

  const handleGSTAction = async () => {
    if (!isVerified)
      return Alert.alert("Verify OTP first");
    ...
    try {
      ...
      /* ---- If GST not verified before → uniqueness check ---- */
      /* ---- If GST not verified before → uniqueness check ---- */
if (!gstVerified) {
...
if (gstDoc.exists) {
  const existingData = gstDoc.data();
  ...
  if (existingUid !== user.uid) {
    return Alert.alert("GST already registered");
      ...
      if (!data?.valid)
        return Alert.alert("GST Invalid");

      const updatedName = data.tradeName || "";
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      await firestore()
        ...
        .doc(user.uid)
        .update({
          address: gstVerified ? address : address,
          email,
          updatedAt:
            firestore.FieldValue.serverTimestamp(),
        });
  ...
  const handleChangePassword = async () => {
    ...
    if (!newPassword || newPassword.length < 6)
      return Alert.alert("Password must be 6+ chars");
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#0B1F3B" }}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <TextInput style={styles.input} value={contractorName} editable={false} />
